...
export default function SecurityPage() {
  return (
      <DocPage title="Security">
        ...
        <ul>
          <li><code>apiEndpoint</code></li>
          <li>UI props (title, theme, variant, etc.)</li>
          <li>Runtime flags (memory, rag, tools, stream)</li>
          <li>Optional <code>userId</code>, <code>sessionId</code>, <code>metadata</code></li>
        </ul>
        <p>They must <strong>never</strong> receive:</p>
        <ul>
        ...
        <p>Your API route (via <code>@kortex/config</code>) owns:</p>
        <ul>
        ...
        <ul>
          <li>Never import <code>@kortex/config</code>, <code>@kortex/openai</code>, or provider SDKs in frontend bundles</li>
          <li>Validate <code>userId</code> and <code>sessionId</code> on the server for production apps</li>
          <li>Rate-limit and authenticate your <code>apiEndpoint</code> route</li>
          <li>Use HTTPS for remote <code>apiEndpoint</code> URLs (React Native, cross-origin)</li>
        </ul>
